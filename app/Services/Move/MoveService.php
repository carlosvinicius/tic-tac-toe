<?php

namespace App\Services\Move;

use App\Services\Move\MoveInterface;
use App\Services\Move\Rules\RobotWinnerMoveRule;
use App\Services\Move\Rules\UserWinnerMoveRule;
use App\Services\Move\Rules\CenterMoveRule;
use App\Services\Move\Rules\EmptyMoveRule;
use App\Services\Move\Rules\UserNextMoveRule;

/**
 * @author Carlos Vinicius carlos@cvps.com.br
 */
class MoveService implements MoveInterface
{
    /**
     * Makes a move using the $boardState
     * $boardState contains 2 dimensional array of the game field
     * X represents one team, O - the other team, empty string means field is not yet taken.
     * example
     * [['X', 'O', '']
     * ['X', 'O', 'O']
     * ['', '', '']]
     * Returns an array, containing x and y coordinates for next move, and the unit that now occupies it.
     * Example: [2, 0, 'O'] - upper right corner - O player
     *
     * @param array $boardState Current board state
     * @param string $playerUnit Player unit representation
     *
     * @return array
     */
    public function makeMove(array $boardState, string $playerUnit = 'X') : array
    {
        $robotWinnerMoveRule = new RobotWinnerMoveRule();
        $userWinnerMoveRule  = new UserWinnerMoveRule();
        $centerMoveRule      = new CenterMoveRule();
        $emptyMoveRule       = new EmptyMoveRule();
        $userNextMoveRule    = new UserNextMoveRule();

        $robotWinnerMoveRule->setNext($userWinnerMoveRule);
        $userWinnerMoveRule->setNext($centerMoveRule);
        $centerMoveRule->setNext($userNextMoveRule);
        $userNextMoveRule->setNext($emptyMoveRule);

        $bestMove  = $robotWinnerMoveRule->getBestMove($boardState, $playerUnit);
        $robotUnit = $playerUnit == 'X' ? 'O' : 'X';

        array_push($bestMove, $robotUnit);

        return $bestMove;
    }
}
